...
export const AdminUserStatsPanel = ({ user }) => {
  return (
    <>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-2">
         <StatCard icon={MessageSquare} value={user.comments_count} label="Comentarios" iconColor="text-indigo-500" valueClass="text-2xl" />
         <StatCard icon={PlusSquare} value={user.items_count} label="Items Creados" iconColor="text-pink-500" valueClass="text-2xl" />
        {user.role === 'admin' && (
          <StatCard icon={Eye} value={user.reviewed_proposals_count} label="Revisiones" iconColor="text-teal-500" valueClass="text-2xl" />
        )}
         <StatCard icon={Key} value={user.sessions_count} label="Sesiones Activas" iconColor="text-orange-500" valueClass="text-2xl" />
      </div>
    </>
  );
};
