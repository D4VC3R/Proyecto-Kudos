...
export const StatCard = ({ icon: Icon, value, label, containerClass = "bg-slate-50 border-slate-100", iconColor = "text-blue-500", valueClass = "text-3xl" }) => {
  return (
    <AnimatedCard className={clsx("border p-5 rounded-3xl flex flex-col items-center justify-center text-center", containerClass)}>
      ...
      <span className="text-xs font-bold text-slate-600 uppercase mt-1">{label}</span>
    </AnimatedCard>
  );
};
